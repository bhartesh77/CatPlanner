...
export const Planner = () => {
  ...
  useEffect( () => {
    ...
    if(CountComplete === Tasks.length && Tasks.length > 0) setActivateVideo( true )
  ...
  const AddTaskButtonHandler = () => {
    ...
    TempTasks.push({ Id: uuid(), Task: InputTaskValue.trim(), Complete: false })
    Tasks.forEach( (task) => {
  ...
  return (
    <div className={classes.Planner}>
      ...
      <div className={classes.PlannerDiv}>
        {ActivateVideo && <video autoPlay loop className={classes.Video}>
        ...
        <ul className={classes.TaskList}>
          {Tasks.map((task) => {
            return (
              <li key={task.Id} className={classes.TaskLi} style={{opacity: task.Complete ? '60%' : '100%'}}>
                <div
                  className={classes.CompleteTaskLine}
                  style={{opacity: task.Complete ? '40%' : '0'}}
                ></div>
                <span className={classes.LiContentSpan} style = {{color: ActivateVideo ? '#000' : '#fff', fontWeight: ActivateVideo ? 'bold' : '500'}}>{task.Task}</span>
                <button
